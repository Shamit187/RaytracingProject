...
{
    auto ray = Ray(point, light - point);
...
Color raycast(const Ray &ray, const int iterationDeapth)
{
    ...
    float depth = 1000000.0f;
    Color color = Color(0.0f, 0.0f, 0.0f);
    Intersection intersection;

    // first checkerboard intersection
    ...
    float lambert = 0, phong = 0;
    // check if lights are visible
    for (auto normalLight : normalLights)
    {
        if(!checkLightVisibility(intersection.point, normalLight.position)) continue;
    ...
    { 
        if(!checkLightVisibility(intersection.point, spotLight.position)) continue;
        // check cutoff
        auto lightDirection = (spotLight.position - intersection.point).normalize();
        // check if angle with light is less than cutoff
        if (lightDirection.dot(spotLight.direction) < cos(spotLight.cutoff)) continue;
        auto normal = intersection.normal.normalize();
    ...
    for (auto sphere : spheres){
        ...
        intersection = ray.intersect(sphere);
        if(intersection.valid == false) continue;
